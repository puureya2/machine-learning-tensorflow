...
train_dataset = image_dataset_from_directory(
    ...
    validation_split=0.9,  # Use only 10% for training
    subset="training",
    seed=42
)
...
val_dataset = image_dataset_from_directory(
    ...
    image_size=(224, 224),
    batch_size=8,
    validation_split=0.1,  # Use only 10% for validation
    subset="validation",
    seed=42
)

# Train ResNet50 with reduced complexity
resnet_base = ResNet50(weights='imagenet', include_top=False, input_shape=(224, 224, 3))
resnet_model = modify_base_model(resnet_base, num_classes=3, freeze_layers=150)
resnet_history = compile_and_train_model(
    resnet_model,
    train_dataset,
    val_dataset,
    save_path=f'{root}/Models/resnet50_best_checkpoint.keras',
    epochs=2  # Only 2 epochs for demo
)
